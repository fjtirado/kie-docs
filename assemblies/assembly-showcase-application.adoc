...
* `KieLoginModule` is configured in `standalone-full.xml`. This required to connect to the {KIE_SERVER}. For more information about configuring the {KIE_SERVER}, see {URL_INSTALLING_AND_CONFIGURING}#assembly-planning[_{PLANNING_INSTALL}_].
...
* The *IT_Orders* sample project has been imported in {CENTRAL} and deployed to the {KIE_SERVER}. For more information about case management, see {URL_GETTING_STARTED_CASES}[_{GETTING_STARTED_CASES}_].
