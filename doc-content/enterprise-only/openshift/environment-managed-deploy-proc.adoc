[id='environment-managed-deploy-proc_{context}']
...
** Replace `<maven-repo-url>` with the URL for a Maven repository where the necessary KJAR files are available.
...
** *KIE Server Mode* (`KIE_SERVER_MODE`): In the `{PRODUCT_INIT}{ENTERPRISE_VERSION_SHORT}-prod.yaml` template the default value is `PRODUCTION`; in this mode, you cannot deploy `SNAPSHOT` versions of KJAR artifacts and cannot change versions of an artifact in an existing container. (To deploy a new version with `PRODUCTION` mode, you can create a new container on the same {KIE_SERVER}). To deploy `SNAPSHOT` versions or to change versions of an artifact in an existing container, set this parameter to `DEVELOPMENT`.
