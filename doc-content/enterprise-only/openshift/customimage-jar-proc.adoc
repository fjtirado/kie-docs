...
. Develop a custom library that works with the {KIE_SERVER}. You can use the following documentation and examples to develop the library:
** {URL_MANAGING_KIE_SERVER}/kie-server-extensions-con_execution-server#kie-server-extensions-con_execution-server[{KIE_SERVER} capabilities and extensions] in _{MANAGING_KIE_SERVER}_.
...
For instructions about deploying the {KIE_SERVER} using the operator, see xref:operator-deploy-kieserver-proc_{context}[].